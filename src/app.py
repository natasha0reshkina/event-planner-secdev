...
async def create_event(payload: dict):
    ...
    _DB[new_id] = {"title": inp.title, "place": inp.place, "date": inp.date_.isoformat(), "note_len": safe_note_len(inp.note)}
    return {"id": new_id}
...
async def update_event(event_id: str, payload: dict):
    ...
    try:
        ...
        return problem(422, "Неверные данные", str(e))
    _DB[event_id].update({"title": inp.title, "place": inp.place, "date": inp.date_.isoformat(), "note_len": safe_note_len(inp.note)})
    return {"ok": True}
